...
class TestApp(EWrapper, EClient):
    ...
    def running_list(self, price: float):
        ...
        self.data_counter += 1
        if self.data_counter < self.mov_avg_length_tf1_s:
            return
        while len(self.data) > self.mov_avg_length_tf1_s:
            self.data.pop(0)
    ...
    def running_list1(self, price: float):
        ...
        if self.data_counter1 < self.mov_avg_length_tf1_f:
            return
        while len(self.data1) > self.mov_avg_length_tf1_f:
            self.data1.pop(0)
    ...
    def calc_indicator(self):
        ...
        df_indicator['indicator'] = TA.SMA(df_indicator, self.mov_avg_length_tf1_s) # choose indicator here
    ...
    def calc_indicator1(self):
        ...
        df_indicator1['indicator1'] = TA.SMA(df_indicator1, self.mov_avg_length_tf1_f) # choose indicator here
    ...
    def decision_engine(self):
        if self.prev_indicator != 0:
            ...
            if self.prev_indicator < self.indicator:
                self.signal = 'LONG'
            elif self.prev_indicator > self.indicator:
                self.signal = 'SHORT'
            else:
    ...
    def decision_engine_crossover(self):
        if self.prev_indicator != 0:
            self.prev_signal = self.signal
            if self.prev_indicator > self.indicator and self.prev_indicator < self.indicator:
                self.signal = 'LONG'
            elif self.prev_indicator < self.indicator and self.prev_indicator > self.indicator:
                self.signal = 'SHORT'
            else:
                self.signal = self.prev_signal
