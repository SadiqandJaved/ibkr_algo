...
class TestApp(EWrapper, EClient):
    ...
    def decision_engine_crossover(self):
        if self.prev_indicator_tf1_s != 0 and self.prev_indicator_tf1_f != 0: # need to have atleast 2 signals (previous & current) to make a decision
            self.prev_signal = self.signal
            # Only using roc for long for now, need to figure out the short condition with a OR
            if (self.roc_tf1_s > 0) and (self.roc_tf1_f > 0) and (self.prev_indicator_tf1_s > self.prev_indicator_tf1_f) and (self.indicator_tf1_s < self.indicator_tf1_f):
                self.signal = 'LONG'
                print(f'\nGoing LONG because the following conditions were MET in decision engine\n'
                      f'    {self.roc_tf1_s} (roc_tf1_s) > 0 and {self.roc_tf1_f} (roc_tf1_f) > 0\n'
            ...
            #If we are already LONG, go SHORT as soon as ROC < 0 (don't have to wait for crossover signal)
                ...
                if (self.roc_tf1_s < 0) or (self.roc_tf1_f < 0) or ((self.prev_indicator_tf1_s < self.prev_indicator_tf1_f) and (self.indicator_tf1_s > self.indicator_tf1_f)):
                    ...
                    print(f'\nGoing SHORT because the following conditions were MET in decision engine\n'
                          f'    {self.roc_tf1_s} (roc_tf1_s) or {self.roc_tf1_f} (roc_tf1_f) or\n'
                          f'    {self.prev_indicator_tf1_s} (prev_tf1_s) < {self.prev_indicator_tf1_f} (prev_tf_f) and\n'
                          f'    {self.indicator_tf1_s} (tf1_s) > {self.indicator_tf1_f} (tf1_f)\n)')
            #Use crossover signal to start a SHORT position (only if we are not already LONG)
    ...
    def tickDataOperations_req(self):
        ...
        # futures contract
        self.contract.symbol = 'TQQQ'   #ES
        self.contract.secType = 'STK'   #FUT
        self.contract.exchange = 'SMART'  #GLOBEX
