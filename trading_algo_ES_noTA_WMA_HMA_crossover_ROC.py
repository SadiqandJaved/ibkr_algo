...
class TestApp(EWrapper, EClient):
    ...
    def WMA(self, df, period: int = 9, column: str = "close"):
    ...
    def HMA(self, df, period: int = 9):
        import math

        half_length = int(period / 2)
        sqrt_length = int(math.sqrt(period))

        df["wmaf"] = self.WMA(df, period=half_length)
        df["wmas"] = self.WMA(df, period=period)
        df["deltawma"] = 2 * df["wmaf"] - df["wmas"]
        df["hma"] = self.WMA(df, column="deltawma", period=sqrt_length)
        hma = self.WMA(df, column="deltawma", period=sqrt_length)
    ...
    def calc_indicator_tf1_s(self):
        ...
        df_indicator_tf1_s['indicator_tf1_s'] = self.WMA(df_indicator_tf1_s, self.mov_avg_length_tf1_s) # choose indicator here
        self.indicator_tf1_s = df_indicator_tf1_s['indicator_tf1_s'].iloc[-1]
        print("\nData_TF1_S =", self.data_tf1_s, "\n", "df_indicator_tf1_s =\n", df_indicator_tf1_s, "\n", "indicator_tf1_s =", self.indicator_tf1_s, "\n")
    ...
    def calc_indicator_tf1_f(self):
        ...
        df_indicator_tf1_f['indicator_tf1_f'] = self.HMA(df_indicator_tf1_f, self.mov_avg_length_tf1_f) # choose indicator here
    ...
    def calc_indicator_tf2_s(self):
        ...
        df_indicator_tf2_s['indicator_tf2_s'] = self.WMA(df_indicator_tf2_s, self.mov_avg_length_tf2_s) # choose indicator here
    ...
    def calc_indicator_tf2_f(self):
        ...
        df_indicator_tf2_f['indicator_tf2_f'] = self.HMA(df_indicator_tf2_f, self.mov_avg_length_tf2_f) # choose indicator here
...
def main():
    app = TestApp()
    app.connect("127.0.0.1", port=7497, clientId=1005)  # make sure you have a different client id for each ticker
