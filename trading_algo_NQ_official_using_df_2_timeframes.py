...
# Using TF1 as the slower time frame
TICKS_PER_CANDLE_TF1 = 10  #10 #34 #55 #89 #144 #233 #377
MOVING_AVG_PERIOD_LENGTH_TF1_S = 9 #9 #14 # slow timeframe (WMA)
MOVING_AVG_PERIOD_LENGTH_TF1_F = 9 #5 #9 # fast timeframe (WMA)
# Using TF2 as the larger time frame to go LONG when both TF1 & TF2 timeframes are in agreement
TICKS_PER_CANDLE_TF2 = 20 #5 #10 #34 #55 #89 #144 #233 #377
MOVING_AVG_PERIOD_LENGTH_TF2_S = 14 #9 #14 (WMA) 
MOVING_AVG_PERIOD_LENGTH_TF2_F = 14 #5 #9 (WMA)
...
class TestApp(EWrapper, EClient):
        ...
        if self.prev_indicator_tf1_s != 0 and self.prev_indicator_tf1_f != 0: # need to have atleast 2 signals (previous & current) to make a decision
            ...
            if (self.prev_signal == "SHORT"):
                if (self.roc_tf1_s > 0) or (self.roc_tf1_f > 0) or \
                        (self.roc_tf2_s > 0) or (self.roc_tf2_f > 0) or \
    ...
    def tickDataOperations_req(self):
        ...
        # futures contract
        self.contract.symbol = 'NQ'   #ES #TQQQ
...
def main():
    app = TestApp()
    app.connect("127.0.0.1", port=7497, clientId=1012)  # make sure you have a different client id for each ticker
